...
/*   Updated: 2025/09/03 16:11:49 by skuik            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void add_redir_token(t_token *tok, t_cmd_builder *b)
{
    ...
    if (tok->type == T_REDIR_IN)
        r = new_redir(tok->next, REDIR_IN);
    else if (tok->type == T_REDIR_OUT)
        r = new_redir(tok->next, REDIR_OUT);
    else if (tok->type == T_REDIR_APPEND)
        r = new_redir(tok->next, R_APPEND);
    else if (tok->type == T_REDIR_HEREDOC)
        r = new_redir(tok->next, R_HEREDOC);
    else
        return;
    if (r)
        list_add_back(&b->redirs, r);
}
...
void finalize_cmd_builder(t_cmd_builder *b, t_command **out)
{
    ...
    b->cmd->redirs = list_to_redir_array(b->redirs);
    b->cmd->arg_count = list_size(b->args);
    b->cmd->redir_count = list_size(b->redirs);
    *out = b->cmd;
}
